...
> | 文件 | 内容 |
> |------|------|
> | `GameConstants.cs` | 全部数值常量（40/250/50/3、移速 0.3/0.5 等）+ ID 无效值，唯一定义处 |
...
- AD 左右移动、W 跳跃、S 跳下**不走契约**：角色控制器 + Mirror 位置同步；权威模式在此写死为**客户端权威（本地控制自己的角色，程序 1 只做同步与反作弊性校验）**
...
## 3. 功能 → 契约成员对照（覆盖 PDF 全部功能）

### 基础功能

| PDF 功能 | 契约成员 | 程序 1 职责 | 程序 2 职责 |
|----------|----------|-------------|-------------|
| 躲藏者移速 0.3 / 抓捕者 0.5 | `GameConstants.HiderMoveSpeed / SeekerMoveSpeed` | 校验异常速度（可选） | 控制器读常量 |
| 躲藏者/抓捕者：AD 左右移动、W 跳跃、S 跳下 | （不走契约，客户端权威） | 位置同步 | 角色控制与移动手感 |
...
| 每 50s 随机变身 + 隐身无敌 3s | `Events.OnHiderTransformed(TransformInfo)`（`invulnerableUntil` 为服务端时刻，抵抗延迟）；`HiderState.Invisible` | 定时触发、随机物品、无敌窗口 | 换外观 + 隐身 VFX，剩余时长 = `invulnerableUntil - NetworkTime.time` |
| 抓捕者心跳（被动） | `Events.OnHeartbeatPulse(HeartbeatPulse)`（含 `serverTime` 对齐节拍） | 固定节奏广播 | 范围内物体跳动动画；躲藏者按 W 跟节奏跳 |
